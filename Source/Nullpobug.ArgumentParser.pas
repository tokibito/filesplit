...
type
  ...
  ENoMatchArgument = class(EParseError);  // 一致する引数がありません
  EInvalidArgument = class(EParseError);  // 不正な引数です
  EParameterMissing = class(EParseError);  // パラメータが不足しています
  ENoSuchArgument = class(EParseError);  // そのような引数はありません
  ...
  IParseResult = interface
    ...
    function Arguments: TList<String>;
  end;

  TParseResult = class(TInterfacedObject, IParseResult)
  private
  ...
  public
    ...
    destructor Destroy; override;
    function ParseArgs: IParseResult; overload;
    function ParseArgs(TargetArgs: TList<String>): IParseResult; overload;
...
function TParseResult.Arguments: TList<String>;
(*
  値のリストを返す
 *)
begin
  Result := FUnnamedValues;
end;
(* End of TParseResult *)
...
function TArgumentParser.ParseArgs: IParseResult;
var
  Params: TList<String>;
begin
  (* 実行ファイル名を除いたパラメータを取得 *)
...
function TArgumentParser.ParseArgs(TargetArgs: TList<String>): IParseResult;
var
